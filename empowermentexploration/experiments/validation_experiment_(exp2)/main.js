...
{		if (hide == 'page1') {
		if (document.getElementById('mturk').value == '') {
			//alert('Please enter the participants' ID.');
			alert("Per favore, inserisci l'ID del partecipante.");
			return;
		}
		else {
...
// Exploring phase
var forcedElements = [[0,1], [2,3], [4,1], [4,3], [0,2], [6,7], [5,0], [7,1], [7,8], [1,3]]; // 0: water, 1: fire, 2: earth, 3: air, 4++: additional elements
var roundIndex = 0; // Index of the current round
var results = ["vapore", "polvere", "none", "nuvola", "fango", "none", "none", "mattone", "none", "energia"]
var forcedElement1 = forcedElements[roundIndex][0];
var forcedElement2 = forcedElements[roundIndex][1];
var elements= [];
var rules=[];
...
function drawelements()
{
	...
	for(var i=0; i<drawelem.length; i++) 
	{
		...
		// Check if element is one of the forced elements
		if (
			(roundIndex === 0 && (addn[i] === forcedElement1 || addn[i] === forcedElement2)) ||
			(roundIndex > 0 && (addn[i] === forcedElements[roundIndex][0] || addn[i] === forcedElements[roundIndex][1]))
		  ) {
			elems += ' style="border: 4px solid red; padding: 4px; margin-right: 10px; box-sizing: content-box; display: inline-block; vertical-align: middle;"';
		} 
		elems=elems+'title= "'+drules[i]+'"';
		elems=elems+' onclick="add('+addn[i]+')" style="margin-right: 10px;  display: inline-block; vertical-align: middle;">';
	}
...
function add(which)
{
	...
	{	
		...
		if(which !== forcedElement1 && which !== forcedElement2) {
			alert("Per favore, seleziona gli elementi evidenziati.")
			//alert("Please select the marked elements.")
			return;
		}
		var drawelem=getBaseElements().concat(elements);
		if ((forcedElement1 !== forcedElement2) && (current.includes(drawelem[which]))) {
...
{	
	...
	{
		alert("Per favore, seleziona due elementi.")
		...
		if (outcome=="none")
		{
			var outtext="Questo esperimento non ha creato nuovi elementi.";
		...
		if (outcome!="none")
		{
          ...
          {
			var outtext="Hai scoperto di nuovo "+outcome+".";
          	//var outtext="You rediscovered "+outcome+".";
          }else{
			var outtext="Fantastico! Hai scoperto "+results[roundIndex]+".";
          	//var outtext="Awesome! You discovered "+results[roundIndex]+".";
...
{	
	roundcounter --;
	if(roundcounter == 0){
		clickStart('page4','page5a')
	}
	else{
		roundIndex++;
		forcedElement1 = forcedElements[roundIndex][0];
		forcedElement2 = forcedElements[roundIndex][1];
		drawelements();
		current=["questionmark", "questionmark", "redquestionmark"];
		drawexperiment();

		change("counter", "Turni rimasti: "+roundcounter)
...
function mysubmit()
{
		...
		var validation=2023;
		myDataRef.push({'collect': collect, 'elements': elements, 'bincombinations': linkcombinations, 'bincombinationstring' : linkcombinationstring, 'binanswers': linkanswers, 'empcombinations': empcombinations, 'empcombinationstring': empcombinationstring, 'empowermentanswers': empowermentanswers,'intcombinations': intcombinations, 'intcombinationstring':intcombinationstring, 'intanswers': intanswers, 'validation' : validation})
		senddata()
}
...
function senddata() {
	...
	saveDataArray = {
		'collect': collect, 
		'elements': elements, 
		'bincombinations': linkcombinations, 
		'bincombinationstring' : linkcombinationstring, 
		'binanswers': linkanswers, 
		'empcombinations': empcombinations, 
		'empcombinationstring': empcombinationstring, 
		'empowermentanswers': empowermentanswers,
		'intcombinations': intcombinations, 
		'intcombinationstring':intcombinationstring, 
		'intanswers': intanswers, 
		'id': turkid
	};

	saveText(JSON.stringify(saveDataArray), 'ValidationData'+turkid+'.JSON');
}
...
// randomly assign element 1 and 2	out of a combination
function randomElement(combination) {
	if (randomNum(0,2) == 0) {
		var element1 = predictions["first_string"][combination];
		var element2 = predictions["second_string"][combination];
	}
	else {
		var element1 = predictions["second_string"][combination];
		var element2 = predictions["first_string"][combination];
	}
...
var linkcombinationstring=[];
var currentcombination;
var linkcombinations =[] 
var predictions = df_prediction;
var numberofpredictions = 15;
var predictioncounter = numberofpredictions; 
var linkpredTouched = false; // Check whether an answer was selected
var allpredictions = [];
var linkanswers =[];
var selectedValue = 0;
...
function drawlinkprediction()
{
	...
	if (predictioncounter == 0){
		clickStart("page5", "page6a");
		console.log(allpredictions);
	}
	else{
	// Uncheck radio buttons
	var buttons = document.getElementById("linkpred");
	var selectedRadioButton = buttons.querySelector('input[name="prediction"]:checked');
	if (selectedRadioButton) {
	selectedRadioButton.checked = false;
	}
	selectedValue = 0;
	predictioncounter --;
	currentcombination = randomNum(0,60);
	while(allpredictions.includes(currentcombination)){
		currentcombination = randomNum(0,60);
	}
	allpredictions.push(currentcombination);
	
	var elementassingment = randomElement(currentcombination);
	var first_element = elementassingment[0];
	var second_element = elementassingment[1];

	currentprediction=[first_element, second_element, "redquestionmark"]
	linkcombinationstring.push([first_element, second_element, predictions["success"][currentcombination]]);
	...
	linkcombinations.push(currentcombination) 
	}
}
drawlinkprediction();
...
function linksubmit()
{	
	var selectedRadioButton = document.querySelector('input[name="prediction"]:checked');
	if(!selectedRadioButton){
		...
		linkanswers.push(parseInt(selectedValue));
		drawlinkprediction();
	}
}


// Empowerment prediction
var empquestioncounter = numberofpredictions;
var empowermentanswers = [];
var empcombinationstring = [];
var empcombinations = [];
var selectedValue = 0;
...
{	// page change 6 to 7
	...
	// Uncheck radio buttons
	var buttons = document.getElementById("emppred");
	var selectedRadioButton = buttons.querySelector('input[name="prediction"]:checked');
	if (selectedRadioButton) {
	selectedRadioButton.checked = false;
	}
	...
	currentprediction=[first_element, second_element, "redquestionmark"]
	empcombinationstring.push([first_element, second_element, predictions["success"][currentcombination]]);
	var empprops='<img src="inventpic/'+currentprediction[0]+'.png" width="50px" height="50px"  title = '+currentprediction[0]+'>&nbsp&nbsp&nbsp'+
			  '<img src="proppics/plus.png" width="35px" height="35px">&nbsp&nbsp&nbsp'+
	          '<img src="inventpic/'+currentprediction[1]+'.png" width="50px" height="50px" title = '+currentprediction[1]+'>&nbsp&nbsp&nbsp'+
	   		  '<img src="proppics/equals.png" width="35px" height="35px">&nbsp&nbsp&nbsp'+
	   		  '<img src="proppics/'+currentprediction[2]+'.png" width="35px" height="50px">&nbsp&nbsp&nbsp';
	change('empexp', empprops);
	empcombinations.push(currentcombination)
	}
}
drawempquestion();
...
function submitemp(){
	...
	if(!selectedRadioButton){
		...
		selectedValue = selectedRadioButton.value;
		console.log(selectedValue);
		empowermentanswers.push(parseInt(selectedValue));
		drawempquestion();
	}
}

// Interest prediction
var intquestioncounter = numberofpredictions;
var intcombinationstring = [];
var intanswers = [];
var intcombinations = [];
var currentcombination1;
var currentcombination2;



function drawintquestion()
{	// page change 7 to 8
	if(intquestioncounter == 0){
	...
	var selectedRadioButton = buttons.querySelector('input[name="combination"]:checked');
	if (selectedRadioButton) {
	selectedRadioButton.checked = false;
	}
	intquestioncounter --;
	currentcombination1 = randomNum(0,60);
	while(allpredictions.includes(currentcombination1)){
		currentcombination1 = randomNum(0,60);
	}
	allpredictions.push(currentcombination1);

	var elementassingment1 = randomElement(currentcombination1);
	var first_element1 = elementassingment1[0];
	var second_element1 = elementassingment1[1];
	
	currentprediction1=[first_element1, second_element1, "redquestionmark"]
	var intprops1='<img src="inventpic/'+currentprediction1[0]+'.png" width="50px" height="50px"  title = '+currentprediction1[0]+'>&nbsp&nbsp&nbsp'+
			  '<img src="proppics/plus.png" width="35px" height="35px">&nbsp&nbsp&nbsp'+
	          '<img src="inventpic/'+currentprediction1[1]+'.png" width="50px" height="50px" title = '+currentprediction1[1]+'>&nbsp&nbsp&nbsp'+
	   		  '<img src="proppics/equals.png" width="35px" height="35px">&nbsp&nbsp&nbsp'+
	   		  '<img src="proppics/'+currentprediction1[2]+'.png" width="35px" height="50px">&nbsp&nbsp&nbsp';

	currentcombination2 = randomNum(0,60);
	while(allpredictions.includes(currentcombination2)){
		currentcombination2 = randomNum(0,60);
	}
	allpredictions.push(currentcombination2);

	var elementassingment2 = randomElement(currentcombination2);
	var first_element2 = elementassingment2[0];
	var second_element2 = elementassingment2[1];
	
	currentprediction2=[first_element2, second_element2, "redquestionmark"]
	intcombinationstring.push([[first_element1, second_element1, predictions["success"][currentcombination1]],[first_element2, second_element2, predictions["success"][currentcombination2]]]);
	var intprops2='<img src="inventpic/'+currentprediction2[0]+'.png" width="50px" height="50px"  title = '+currentprediction2[0]+'>&nbsp&nbsp&nbsp'+
			  '<img src="proppics/plus.png" width="35px" height="35px">&nbsp&nbsp&nbsp'+
	          '<img src="inventpic/'+currentprediction2[1]+'.png" width="50px" height="50px" title = '+currentprediction2[1]+'>&nbsp&nbsp&nbsp'+
	   		  '<img src="proppics/equals.png" width="35px" height="35px">&nbsp&nbsp&nbsp'+
	   		  '<img src="proppics/'+currentprediction2[2]+'.png" width="35px" height="50px">&nbsp&nbsp&nbsp';
	change("intexp1", intprops1);
	change("intexp2", intprops2);
	intcombinations.push([currentcombination1, currentcombination2])
	}
}
drawintquestion();
...
function submitint(){
	...
	var selectedRadioButton = document.querySelector('input[name="combination"]:checked');
	if(!selectedRadioButton)
	{
		alert("Per favore, seleziona una combinazione prima di andare avanti.");
  		//alert("Please select a combination before submitting.");
	}
	else{
	var selectedCombination = selectedRadioButton.value;
	var selectedValue = parseInt(selectedCombination.split(" ")[1]);
	intanswers.push(selectedValue);
	drawintquestion();
	}
}
