...
class PlayerData():
    ...
    """
    def __init__(self, group='children', data_source='human', memory_type=1):
        """Initializes little alchemy player data.

        Args:
            group (str, optional): States group of participants: 'children', 'adults'. Defaults to 'children'.
        ...
        if data_source == 'human':
            if memory_type == 1 or memory_type == 2:
                ...
                memory = False
            self.data = data_handle.get_player_data(memory=memory, group=group)      
        else:
            self.data = data_handle.get_simulation_data(data_source, memory_type)         
