...
from empowermentexploration.regression.rand_model import RandModel
...
class ModelValues():
    ...
    """
    def __init__(self, player_data, time, group, split_version, vector_version, model_type, memory_type, matched, emp_version, z_score=True, stepsused=None):
        ...
        Args:
            ...
            group (str): Group of participants. Either 'children' or 'adults'.
            ...
            stepsused (int, optional): Number of trials that are considered. Defaults to None.
        """
        ...
        for run in range(runs):
            ...
            cbv_model = CBVModel()
            cbv_model.reset()
            cbu_model = CBUModel() #normal cbu model.
            cbu_model.reset()
            rec_model = RecencyModel()
            rec_model.reset()
            sim_model = SimModel()
            bin_model = BinModel(split_version, vector_version)
            rand_model = RandModel()
            if emp_version == "elements":
                emp_model = EmpModel(split_version, vector_version) #need to decide if element or direct empowerment model (values) is used. Dependent on the gametreefile (need to be changed if other model should be used).
            elif emp_version == "values":
                emp_model = EmpdirectModel(split_version, vector_version)
            else:
                raise ValueError('Undefined version: "{}". Use "values" or "elements" instead.'.format(emp_version))


            truebin_model = TrueBinModel()
            trueemp_model = TrueEmpModel()
            ...
            for trial in player_data:
                ...
                models = [('cbv', cbv_model), ('cbu', cbu_model), ('rec', rec_model), ('sim', sim_model), ('bin', bin_model), ('emp', emp_model), ('truebin', truebin_model), ('trueemp', trueemp_model), ('rand', rand_model)]
            ...
            if run == 0:
                ...
                self.data_complete = pd.concat([self.data_complete, self.data], ignore_index=True)
        # store as csv file
        log.store_regression_data(self.data_complete, time, z_score, model_type, memory_type, group, stepsused)
