...
if __name__ == '__main__':
    ...
    # select group of participants: children, adults
    group = 'children'
    ...
    # plot info for user
    print('\nGet regression data for player data from {}. Save logs to directory empowermentexploration/data/regression/'.format(group))
    if stepsused is None:
        print('Trials considered: all')
    else:
        print('Trials considered: {}'.format(stepsused))
    print('Memory type: {}'.format(memory_type))

    # load player data
    players = PlayerData(group=group, data_source=data_source, memory_type=memory_type)
    ...
    values = ModelValues(players.data, time, group, split_version, vector_version, data_source, memory_type, matched, emp_version, z_score=False, stepsused=stepsused)
